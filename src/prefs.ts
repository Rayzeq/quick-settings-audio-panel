...
export default class QSAPPreferences extends ExtensionPreferences {
	...
	makeExtensionSettingsPage(settings: Gio.Settings): Adw.PreferencesPage {
		...
		perdevice_volume_sliders_group.add_switch("perdevice-volume-sliders-change-button", {
			...
			subtitle: _(
				`This button, added on each device slider, has the same effect as clicking on that device in the main volume slider menu`,
			),
		});
		const perdevice_volume_sliders_change_menu = perdevice_volume_sliders_group.add_switch(
			"perdevice-volume-sliders-change-menu",
			{
				title: _("Replace the 'set as active' button with a submenu"),
				subtitle: _(
					`For devices which have multiple ports, the button will be replaced by a submenu to choose which port will be active`,
				),
			},
		);
		settings.bind(
			"perdevice-volume-sliders-change-button",
			perdevice_volume_sliders_change_menu,
			"sensitive",
		...
		mpris_controllers_group.add_switch("mpris-controllers-are-moved", {
			...
			subtitle: _(
				`Move the media controls from the notifications panel instead of creating a new one`,
			),
		});
		...
		applications_volume_sliders_group.add_switch(
			"applications-volume-sliders-allow-automatic-pactl",
			{
				...
				subtitle: _(
					"This feature might cause some minor lag, but it's necessary to get the real name of Chromium applications and detect the application's current output device",
				),
			},
		);
		...
		widgets_order_group
			.add_reorderable("output-volume-slider", {
				title: _("Speaker / Headphone volume slider"),
			})
			.add_switch("move-output-volume-slider");
		widgets_order_group
			.add_reorderable("perdevice-volume-sliders", {
				title: _("Per-device volume sliders"),
			})
		...
		widgets_order_group
			.add_reorderable("input-volume-slider", {
				title: _("Microphone volume slider"),
			})
		...
		const applications_volume_sliders = widgets_order_group
			.add_reorderable("applications-volume-sliders", {
				title: _("Applications mixer"),
			})
			...
			(found: boolean) => {
				...
				if (!found) {
					subtitle +=
						"\n" +
						_(
							'<span color="darkorange" weight="bold"><tt>pactl</tt> was not found, you won\'t be able to change the output device per application</span>',
						);
				}
		...
		const update_pactl_status = (): [boolean, boolean] => {
			const [pactl_path, found_using_custom_path] = get_pactl_path(settings);
		...
		const perdevice_volume_sliders_filters_group = new FilterPreferencesGroup(
			settings,
			"perdevice-volume-sliders-filters",
			"perdevice-volume-sliders-filter-mode",
			{
				title: _("Per-device sliders filtering"),
				description: _(
					"Allows you to filter the per-device volume sliders. The content of the filters are <b>regexes</b> and are applied to the device's display name and pulseaudio name.",
				),
		...
		const applications_volume_sliders_filters_group = new FilterPreferencesGroup(
			settings,
			"applications-volume-sliders-filters",
			"applications-volume-sliders-filter-mode",
			{
				title: _("Application mixer filtering"),
				description: _(
					"Allows you to filter the applications that show up in the application mixer <b>using regexes</b>",
				),
	...
	private make_profile_renamer(settings: Gio.Settings): PreferencesGroup {
		const group = new PreferencesGroup(settings, {
			...
			description: _(
				"Allows you to rename profiles of audio devices (only effective in the profile switcher)",
			),
		});
...
function PreferencesRowList<T extends Constructor<BasePreferencesRowList & GObject.Object>>(
	Base: T,
) {
	return GObject.registerClass(
		...
		class extends Base {
			...
			add_combobox(
				key: string,
				properties: Partial<Adw.ComboRow.ConstructorProps> & {
			...
			add_file_chooser(
				...
				const chooser_button = new Gtk.Button({
					label: "Choose file...",
					has_frame: false,
				});
				chooser_button.connect("clicked", () => {
					...
					dialog.open(null, null, (_, result) => {
						...
						if (path !== null && path !== undefined) row.text = path;
					});
				});
...
const PreferencesGroup = PreferencesRowList(
...
const ListBox = PreferencesRowList(
	GObject.registerClass(
...
const FilterPreferencesGroup = GObject.registerClass(
	class FilterPreferencesGroup extends PreferencesGroup {
		private _key: string;
		...
		constructor(
			...
			const add_filter_button = new Gtk.Button({
				icon_name: "list-add",
				has_frame: false,
			});
			...
			this.add_combobox(mode_key, {
				...
				subtitle: _(
					"On blocklist mode, matching elements are removed from the list. On allowlist mode, only matching elements will be shown",
				),
				fields: [
...
const ReorderablePreferencesGroup = GObject.registerClass(
	class ReorderablePreferencesGroup extends Adw.PreferencesGroup {
		...
		add_reorderable(
			key: string,
			properties: Partial<Adw.ActionRow.ConstructorProps>,
		): DraggableRowClass {
...
class DraggableRowClass extends Adw.PreferencesRow {
	...
	constructor(
		...
		const drag_handle = new Gtk.Image({
		...
		drag_source.connect("drag-begin", (_source, drag) => {
			...
			this._drag_widget.set_size_request(
				this._header.get_allocated_width(),
				this._header.get_allocated_height(),
			);

			const row_copy = new DraggableRow(settings, "", {
				...properties,
				title: this._header.title,
				subtitle: this._header.subtitle,
			});
	...
	add_subgroup(group: ListBox): DraggableRowClass {
		...
		this._header.add_suffix(
			new Gtk.Image({
				icon_name: "adw-expander-arrow-symbolic",
