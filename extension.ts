...
import {
	...
	AudioProfileSwitcher,
	BalanceSlider,
	MprisList,
	SinkMixer,
} from "./libs/widgets.js";
...
export default class QSAP extends Extension {
	...
	async enable() {
		...
		this._extension_controller = new ExtensionController(
			this.settings,
			this.getLogger(),
			this.InputVolumeIndicator,
			OutputVolumeSlider,
		);
		...
		this.settings.emit(
			"changed::master-volume-sliders-show-current-device",
			"master-volume-sliders-show-current-device",
		);
		...
		this.settings.emit(
			"changed::add-button-applications-output-reset-to-default",
			"add-button-applications-output-reset-to-default",
		);
	...
	_refresh_panel() {
		...
		const create_applications_volume_sliders = this.settings.get_boolean(
			"create-applications-volume-sliders",
		);
		const create_perdevice_volume_sliders = this.settings.get_boolean(
			"create-perdevice-volume-sliders",
		);
		...
		if (
			move_output_volume_slider ||
			move_input_volume_slider ||
			create_mpris_controllers ||
			create_applications_volume_sliders ||
			create_perdevice_volume_sliders ||
			...
			if (panel_type === "separate-indicator") {
				...
				this._panel.box.add_constraint(
			...
			for (const widget of widgets_order) {
				if (widget === "output-volume-slider" && move_output_volume_slider) {
					...
					this._create_app_mixer(
						index,
						this.settings.get_boolean("group-applications-volume-sliders"),
						this.settings.get_string("applications-volume-sliders-filter-mode"),
						this.settings.get_strv("applications-volume-sliders-filters"),
					);
	...
	_create_sink_mixer(
		...
		this._sink_mixer = new SinkMixer(
			this._panel,
			index,
			filter_mode,
			filters,
	...
	_add_reset_applications_output() {
		this._action_application_reset_output = OutputVolumeSlider.menu.addAction(
			_("Reset all applications to default output"),
			() => {
...
class ExtensionController {
	...
	constructor(
		settings: Gio.Settings,
		logger: Console,
		input_volume_indicator: Volume.InputIndicator,
		output_volume_slider: Volume.OutputStreamSlider,
	) {
		...
		this.connect_setting("changed::always-show-input-volume-slider", () => {
			this.set_always_show_input_volume_slider(
				this.settings.get_boolean("always-show-input-volume-slider"),
			);
		});
		this.connect_setting("changed::ignore-virtual-capture-streams", () => {
			this.set_ignore_virtual_capture_streams(
				this.settings.get_boolean("ignore-virtual-capture-streams"),
			);
		});
		this.connect_setting("changed::remove-output-volume-slider", () => {
			this.set_remove_output_volume_slider(
				this.settings.get_boolean("remove-output-volume-slider"),
			);
		});
	}
	...
	private update_renames_list() {
		...
		const add_card = (card: Gvc.MixerCard) => {
			const renames: Record<string, Record<string, [string, string]>> = this.settings
	...
	private set_ignore_virtual_capture_streams(enable: boolean) {
		...
		if (enable && !was_active) {
			...
			this.injection_manager.restoreMethod(
				this.input_volume_slider.constructor.prototype,
				"_shouldBeVisible",
			);
	...
	private set_always_show_input_volume_slider(enable: boolean) {
		...
		if (enable && !was_active) {
			this.connect(this.input_volume_slider, "notify::visible", () =>
	...
	private reset_input_volume_visibility() {
		if (this.input_is_recursing) {
			...
			if (
				this.settings.get_boolean("always-show-input-volume-slider") &&
				!this.input_volume_slider.visible
			) {
